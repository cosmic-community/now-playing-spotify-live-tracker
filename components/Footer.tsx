...
export default function Footer({ config, settings }: FooterProps) {
  const updateInterval = settings?.metadata?.update_interval || 30000
  const updateSeconds = Math.floor(updateInterval / 1000)

  return (
    <footer className="border-t border-gray-800 bg-spotify-black/50 mt-8">
      <div className="container mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-center md:text-left">
            <p className="text-sm text-spotify-gray">
              Updates every {updateSeconds} seconds • Powered by{' '}
              <Link 
              ...
              <div className="flex items-center gap-3">
                {config.metadata.social_links.spotify && (
                  <Link
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-spotify-gray hover:text-spotify-green transition-colors duration-200"
                  >
